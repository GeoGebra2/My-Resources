...
def newton_eq(f, fp, fpp, x0, A, b, initial_stepsize=1.0, alpha=0.5, beta=0.5, tol=1e-8, maxiter=100000):
	...
	for it in range(maxiter):
		#   START OF YOUR CODE
		#print("x: ", x)
		kkt = np.block([[fpp(x), A.T],[A, np.zeros((m, m))]])
		#print("rank: ", np.linalg.matrix_rank(kkt))
		kkta = np.block([[-fp(x)], [np.zeros((m, 1))]])
		
		solution = np.linalg.solve(kkt, kkta)
		#print("kkt: ", kkt)
		#print("kkta: ", kkta)
		#print("solution: ", solution)
		
		v = solution[:x.shape[0]]
		#print("v: ", v)
		#print(solution)
		#print(x)
		t = initial_stepsize
		#print("x_k+1: ", x + t*v)
		#print(f(x + t*v))
		#print(f(x) + alpha*t*(fp(x).T)@v)
		while((f(x + t*v) > (f(x) + alpha*t*(fp(x).T)@v)) or (x + t*v < 0).any()):
			t = beta*t
		#print("t: ", t)
		#print("x_k+1: ", x + t*v)
